...
import { getAllActivities, deleteActivity, updateActivity } from "@/lib/db";
...
export default function Home() {
  ...
  const { syncState, syncActivities, checkPendingActivities } =
    useSyncActivities();
  ...
  const handleUpdateActivity = async (
    ...
    try {
      ...
      await loadActivities(); // Reload all activities to reflect changes
      checkPendingActivities();
    } catch (error) {
  ...
  const [activeTab, setActiveTab] = useState<"timer" | "analytics" | "history">(
    "timer",
  );
  ...
  const totalProdSecs = todaysActivities
    .filter((a) => a.type === "positive")
    .reduce((s, a) => s + calcClampedDur(a), 0);
  const totalDistSecs = todaysActivities
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800 overflow-hidden font-sans">
      ...
      <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 lg:px-12 shrink-0 z-10 w-full shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <RefreshCw
              className={`w-5 h-5 ${syncState.isSyncing ? "animate-spin text-blue-500" : ""}`}
            />
                ...
                >
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 shrink-0">
                    <div className="bg-[#f0f9ff] text-blue-900 p-4 sm:p-5 rounded-2xl shadow-sm border border-blue-100 flex flex-col justify-center">
                      <p className="text-blue-600/80 text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-1">
                        Total Activities
                      </p>
                    ...
                    <div className="bg-[#f0fdf4] text-emerald-900 p-4 sm:p-5 rounded-2xl shadow-sm border border-emerald-100 flex flex-col justify-center">
                      <p className="text-emerald-600/80 text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-1">
                        Productive Today
                      </p>
                      <h3 className="text-xl sm:text-2xl font-bold">
                        {formatH(totalProdSecs)}{" "}
                    ...
                    <div className="bg-[#fff1f2] text-rose-900 p-4 sm:p-5 rounded-2xl shadow-sm border border-rose-100 flex flex-col justify-center">
                      <p className="text-rose-600/80 text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-1">
                        Distracting Today
                      </p>
                      <h3 className="text-xl sm:text-2xl font-bold">
                        {formatH(totalDistSecs)}{" "}
                        <span className="text-[10px] sm:text-sm font-medium opacity-70">
                          hrs
                        </span>
                      </h3>
                  ...
                  <div className="flex-1 min-h-0 bg-white border border-slate-200 rounded-4xl shadow-sm relative overflow-hidden">
                    <TimerComponent />
                  </div>
                </motion.div>
              )}
              ...
              {activeTab === "history" && (
                ...
                >
                  ...
                  <div className="lg:col-span-2 bg-white rounded-3xl p-6 border border-slate-200 shadow-sm flex flex-col h-full overflow-hidden">
                    <h3 className="text-lg font-bold text-slate-800 mb-6 sticky top-0 bg-white z-10">
                      Activity Logs
                    </h3>
